...
function SidebarTrigger({
    ...
    return (
        <Button
            ...
            size="icon"
            className={cn(
                "hover:bg-accent hover:text-accent-foreground ease-snappy size-8",
                className,
            )}
            onClick={(event) => {
                onClick?.(event);
...
function SidebarSection({
    ...
    const handlePinItem = React.useCallback(
        (itemId: string, pinned: boolean) => {
            if (onPinItem) {
                onPinItem(itemId, pinned);
            } else {
                togglePin(itemId, pinned);
            }
        },
        [onPinItem, togglePin],
    );
        ...
        <SidebarMenuItem>
            <ContextMenu>
                ...
                <ContextMenuContent className="w-48">
                    <ContextMenuItem onClick={() => onNavigate(basePath)}>
                        {icon}
                        Open
                    </ContextMenuItem>
                    <ContextMenuSeparator />
                            ...
                            <>
                                {sortedItems.map((item) => (
                                    ...
                                    >
                                        ...
                                        isPinned(item.id) ? (
                                            <Circle className="text-accent-positive h-2! w-4! fill-current" />
                                        ) : (
                                ...
                                <ContextMenu key={item.id}>
                                    <ContextMenuTrigger>
                                        <SidebarMenuButton
                                            ...
                                            size="sm"
                                            className={cn(
                                                "relative z-10 px-3 py-1 text-sm hover:bg-transparent",
